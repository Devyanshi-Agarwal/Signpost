...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        if (start.sequenceNum() == model.width() * model.height()) {
            return null;
        }
        PlaceList successors = start.successors();
        ArrayList<Sq> connected = new ArrayList<Sq>();
        for (Place pl : successors) {
            Sq sq = model.get(pl);
            if (start.sequenceNum() != 0) {
                if (start.sequenceNum() + 1 == sq.sequenceNum()) {
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        PlaceList myPredecessors = end.predecessors();
        ArrayList<Sq> connectedSquares = new ArrayList<Sq>();
        for (Place next : myPredecessors) {
            if (model.get(next).connectable(end)) {
                connectedSquares.add(model.get(next));
            }
        }
        if (connectedSquares.size() == 1) {
            return model.get(connectedSquares.get(0));
        }
        return null;
    }
