...
class Place {
    ...
    static class PlaceList extends ArrayList<signpost.Place> {
        /** Initialize empty PlaceList. */
        PlaceList() {
        }

        /** Initialze PlaceList from a copy of INIT. */
        PlaceList(List<signpost.Place> init) {
            super(init);
        }
    }
     ...
     *  created. */
    static signpost.Place pl(int x, int y) {
        assert x >= 0 && y >= 0;
        int s = max(x, y);
        if (s >= _places.length) {
            signpost.Place[][] newPlaces = new signpost.Place[s + 1][s + 1];
        ...
        if (_places[x][y] == null) {
            _places[x][y] = new signpost.Place(x, y);
        }
        return _places[x][y];
    }
    ...
    public int dirOf(signpost.Place place) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof signpost.Place)) {
            return false;
        }
        signpost.Place other = (signpost.Place) obj;
    ...
    private static signpost.Place[][] _places = new signpost.Place[10][10];
