 ...
 */
class Model implements Iterable<signpost.Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                Sq newSq = new Sq(i, j, 0, false, arrowDirection(i, j), -1);
                _board[i][j] = newSq;
                _solnNumToPlace[_solution[i][j]] = pl(i, j);
                _allSquares.add(newSq);
                allNums.set(_solution[i][j] - 1);
                if (_solution[i][j] == 1 || _solution[i][j] == last) {
                    _board[i][j]._sequenceNum = _solution[i][j];
                    _board[i][j]._hasFixedNum = true;
                    _board[i][j]._group = 0;
                }
            }
        }
        if (Arrays.asList(allNums).contains(false)) {
            throw new IllegalArgumentException("Must contain all numbers");
        }
        for (Sq thisSquare : _allSquares) {
            if (thisSquare._successors == null) {
                thisSquare._successors = new PlaceList();
            }
            int x = thisSquare.x;
            int y = thisSquare.y;
            int dir = thisSquare._dir;
            thisSquare._successors = allSuccessors(x, y, dir);
            for (Place successors : thisSquare.successors()) {
                if (get(successors).predecessors() == null) {
                    get(successors)._predecessors = new PlaceList();
                }
                get(successors)._predecessors.add(thisSquare.pl);
            }
        }
        _unconnected = last - 1;
    }
    ...
    Model(signpost.Model model) {
        ...
        _board = new Sq[_width][_height];
        _allSquares = new ArrayList<>();
        for (Sq old : model._allSquares) {
            Sq newSq = new Sq(old);
            _board[newSq.x][newSq.y] = newSq;
            _allSquares.add(newSq);
        }
        for (Sq thisSquare : _allSquares) {
            if (thisSquare._successors == null) {
        ...
        for (Sq thisSquare : _allSquares) {
            ...
            int y = thisSquare.y;
            int dir = thisSquare._dir;
            thisSquare._head = get(model._board[x][y]._head);
            thisSquare._successor = get(model._board[x][y]._successor);
            thisSquare._predecessor = get(model._board[x][y]._predecessor);
        }
    }
    ...
    boolean autoconnect() {
        ...
        for (Sq square : _allSquares) {
            int mySeqNo = square._sequenceNum;
            if (mySeqNo != 0) {
                for (Place nextSquareP : square.successors()) {
                    Sq nextSquare = get(nextSquareP);
                    if (nextSquare._sequenceNum != 0) {
                        if (square.connectable(nextSquare)) {
    ...
    void solve() {
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                if (_solution[i][j] != width() * height()) {
                    Place place = solnNumToPlace(_solution[i][j]);
                    get(place).connect(solnNumToSq(_solution[i][j] + 1));
                }
            }
        }
        _unconnected = 0;
    ...
    public boolean equals(Object obj) {
        signpost.Model model = (signpost.Model) obj;
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            ...
            if (this._dir == Place.dirOf(this.x, this.y, s1.x, s1.y)) {
                if (s1._predecessor == null
                        && this._successor == null && s1.head() == s1) {
        ...
        boolean connect(Sq s1) {
            ...
            Sq nextS1 = s1;
            while (nextS1 != null) {
                nextS1._head = this._head;
                nextS1 = nextS1._successor;
            }
            if (this.sequenceNum() != 0 && s1.sequenceNum() == 0) {
                Sq thisvar = this;
                while (thisvar._successor != null) {
                    thisvar._successor._sequenceNum = 1 + thisvar._sequenceNum;
                    thisvar = thisvar._successor;
                    releaseGroup(sgroup);
                }
            } else if (s1.sequenceNum() != 0) {
                if (this.sequenceNum() == 0) {
                    Sq tmp = s1;
                    while (tmp._predecessor != null) {
                        tmp._predecessor._sequenceNum = tmp._sequenceNum - 1;
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this.predecessor() == null && next.successor() == null) {
                    ...
                    next._group = next.group(); this._group = -1;
                ...
                Sq predecessor = this; Sq successor = next; boolean fix = false;
                ...
                if (!fix) {
                    ...
                    Sq prev = this;
                    while (prev != null) {
                        prev._sequenceNum = 0; prev = prev._predecessor;
                    }
                }
                fix = false;
                while (successor != null) {
                    if (successor.hasFixedNum()) {
                        fix = true;
                    }
                ...
                if (!fix) {
                    ...
                    Sq nextSq = next;
                    while (nextSq != null) {
                        nextSq._sequenceNum = 0; nextSq = nextSq._successor;
                    }
                }
            }
            Sq nextSq = next;
            while (nextSq != null) {
                nextSq._head = next; nextSq = nextSq._successor;
            }
        }
