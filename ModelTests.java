...
public class ModelTests {
    ...
    public void sqConnectable() {
        ...
        assertTrue("A square is not "
               + "connectable to itself.", s1.connectable(s4));
        assertTrue("These squares should "
                +  "be connectable.", s3.connect(s4));
    }
    @Test
    public void connectNewTests() {
        Model model = new Model(tr(TESTSOLN));
        Sq s1 = model.new Sq(0, 3, 1, true, 3, 0);
        Sq s2 = model.new Sq(2, 1, 0, false, 6, -1);
        Sq s3 = model.new Sq(0, 1, 0, false, 2, -1);
        Sq s4 = model.new Sq(1, 1, 0, false, 1, -1);
        Sq s5 = model.new Sq(3, 3, 0, false, 4, -1);
        Sq s6 = model.new Sq(3, 2, 0, false, 6, -1);
        Sq s7 = model.new Sq(2, 2, 0, false, 7, -1);
        Sq s8 = model.new Sq(1, 3, 0, false, 4, -1);
        Sq s9 = model.new Sq(1, 0, 0, false, 6, -1);
        Sq s10 = model.new Sq(0, 0, 0, false, 8, -1);
        Sq s11 = model.new Sq(0, 2, 0, false, 2, -1);
        Sq s12 = model.new Sq(1, 2, 0, false, 1, -1);
        Sq s13 = model.new Sq(2, 3, 0, false, 4, -1);
        Sq s14 = model.new Sq(2, 0, 0, false, 1, -1);
        Sq s15 = model.new Sq(3, 1, 0, false, 4, -1);
        Sq s16 = model.new Sq(3, 0, 16, true, 1, 0);

        assertTrue("wrong", s8.connect(s12));
        assertTrue("wrong", s12.connect(s13));
        assertTrue("wrong", s1.connect(s2));
        assertTrue("wrong", s3.connect(s4));
        assertTrue("wrong", s2.connect(s3));
        assertTrue("wrong", s11.connect(s7));
        assertTrue("wrong", s7.connect(s8));
        assertTrue("wrong", s4.connect(s5));
        assertTrue("wrong", s5.connect(s6));
        assertTrue("wrong", s6.connect(s11));
    }
    ...
    @Test
    public void disconnectNewTests() {
        Model model = new Model(tr(TESTSOLN));
        Sq s1 = model.new Sq(0, 3, 1, true, 3, 0);
        Sq s2 = model.new Sq(2, 1, 0, false, 6, -1);
        Sq s3 = model.new Sq(0, 1, 0, false, 2, -1);
    ...
    private  static  final int[][] TESTSOLN = {
            { 1, 8, 13, 5 },
            { 11, 12, 7, 6 },
            { 3, 4, 2, 15 },
            { 10, 9, 14, 16 }
